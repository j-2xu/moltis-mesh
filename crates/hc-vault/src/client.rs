    ...
    pub fn new(config: HcVaultConfig, initial_token: Secret<String>) -> Result<Self> {
            ...
            (&config.tls_client_cert, &config.tls_client_key)
        {
            let cert = std::fs::read(cert_path).map_err(|e| {
                HcVaultError::Config(format!("failed to read TLS client cert: {e}"))
            })?;
            let key = std::fs::read(key_path).map_err(|e| {
                HcVaultError::Config(format!("failed to read TLS client key: {e}"))
            })?;
            let identity = reqwest::Identity::from_pkcs8_pem(&cert, &key)
                .map_err(|e| HcVaultError::Config(format!("invalid TLS client identity: {e}")))?;
